...
enum yysymbol_kind_t
{
  ...
  YYSYMBOL_turn = 3,                       /* turn  */
  YYSYMBOL_device = 4,                     /* device  */
  YYSYMBOL_action = 5,                     /* action  */
  YYSYMBOL_iffs = 6,                       /* iffs  */
  YYSYMBOL_bracket = 7,                    /* bracket  */
  YYSYMBOL_time = 8,                       /* time  */
  YYSYMBOL_end = 9,                        /* end  */
  YYSYMBOL_YYACCEPT = 10,                  /* $accept  */
  YYSYMBOL_Scenario = 11,                  /* Scenario  */
...
{
  "\"end of file\"", "error", "\"invalid token\"", "turn", "device",
  "action", "iffs", "bracket", "time", "end", "$accept", "Scenario",
  "line", YY_NULLPTR
};
...
yyreduce:
  ...
  switch (yyn)
    {
  case 4: /* line: turn device action  */
#line 236 "main.y"
                                                                {
								int c=0;
								for(int i=0;i<max_dev;i++){
									if(strcmp((yyvsp[-1].dev),work[i].name)==0){
										if (strcmp((yyvsp[0].act),"on")==0&&work[i].condition==0){
											printf("%s %s",(yyvsp[-1].dev)," is on \n");
											work[i].condition=1;
										}
										else if (strcmp((yyvsp[0].act),"on")==0&&work[i].condition==1){
											printf("%s %s",(yyvsp[-1].dev)," is already on \n");
										}
										else if (strcmp((yyvsp[0].act),"off")==0&&work[i].condition==1){
											printf("%s %s",(yyvsp[-1].dev)," is off \n");
											work[i].condition=0;
										}
										else if(strcmp((yyvsp[0].act),"off")==0&&work[i].condition==0){
											printf("%s %s",(yyvsp[-1].dev)," is already off \n");
										}
										c++;
									}
								}
...
#line 1292 "main.tab.c"
  ...
  case 5: /* line: iffs device action bracket device action bracket  */
#line 264 "main.y"
                                                                {
								D++;
								now_ifs++;
								...
								condition->name_sensor = (yyvsp[-5].dev);
								condition->action_sensor = (yyvsp[-4].act);
								condition->name_device = (yyvsp[-2].dev);
								condition->action_device = (yyvsp[-1].act);
...
#line 1316 "main.tab.c"
  ...
  case 6: /* line: turn device action time  */
#line 286 "main.y"
                                                                {
								D++;
								int c=0;
								...
								exp->name = (yyvsp[-2].dev);
								exp->expaction =(yyvsp[-1].act);
								exp->exptime = (yyvsp[0].tim);
								expect[now_exp]=*exp;
								}
...
#line 1349 "main.tab.c"
    break;


#line 1353 "main.tab.c"
